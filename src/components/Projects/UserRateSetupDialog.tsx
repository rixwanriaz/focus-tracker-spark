...
interface UserRateSetupDialogProps {
  open: boolean;
...
const UserRateSetupDialog: React.FC<UserRateSetupDialogProps> = ({
  open,
  onOpenChange,
  projectId,
  userId,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `${userName}'s hourly rate has been ${currentRate ? 'updated' : 'set'} to $${parseFloat(hourlyRate)}`
      );
      onSuccess();
    } catch (error: any) {
      ...
      <DialogContent className="bg-gradient-to-br from-gray-900 to-gray-800 border-gray-700 text-white shadow-2xl max-w-md w-[95vw] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold flex items-center gap-2">
            ...
            {isUpdate ? 'Update Hourly Rate' : 'Set Hourly Rate'}
          </DialogTitle>
          <p className="text-gray-400 text-sm">
            {isUpdate 
              ? `Update ${userName}'s hourly rate for this project`
              : `Set ${userName}'s hourly rate for this project`
            }
          </p>
          <div className="bg-blue-600/20 border-blue-500/50 rounded-lg p-3 mt-2">
            <p className="text-blue-300 text-xs">
              <strong>Note:</strong> This rate will be used for billing all time tracked by {userName} on this project.
            </p>
          </div>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <AlertDescription className="text-gray-300 text-sm">
              <div className="space-y-1">
                ...
                <ul className="list-disc list-inside space-y-1 text-xs">
                  <li>This rate will be used for billing time tracked on this project</li>
                  <li>Rate can be updated anytime and applies to new time entries</li>
                  <li>Historical financials will not be affected by rate changes</li>
                  <li>Make sure the rate reflects the user's role and experience level</li>
                </ul>
              </div>
            </AlertDescription>
          </Alert>
          ...
          {/* Rate Examples */}
          <div>
            <Label className="text-sm font-medium text-gray-300 mb-2 block">Rate Examples</Label>
